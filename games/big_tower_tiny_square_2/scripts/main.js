...
;('use strict')
{
  ...
  window.RuntimeInterface = class g {
    constructor(d) {
      ...
      !this._useWorker ||
        ('undefined' !== typeof OffscreenCanvas && navigator.userActivation && p()) ||
        (this._useWorker = !1)
      ...
      if ('playable-ad' === this._exportType) {
        ...
        for (let l = 0, n = d.engineScripts.length; l < n; ++l) {
          ...
          this._localFileStrings.hasOwnProperty(k)
            ? (d.engineScripts[l] = { isStringSrc: !0, str: this._localFileStrings[k] })
            : this._localFileBlobs.hasOwnProperty(k) &&
              (d.engineScripts[l] = URL.createObjectURL(this._localFileBlobs[k]))
        }
      }
      d.baseUrl
    ...
    _GetWorkerURL(d) {
      d = this._workerScriptURLs.hasOwnProperty(d)
        ...
        : d.endsWith('/workermain.js') && this._workerScriptURLs.hasOwnProperty('workermain.js')
        ? this._workerScriptURLs['workermain.js']
        : 'playable-ad' === this._exportType && this._localFileBlobs.hasOwnProperty(d.toLowerCase())
    ...
    async CreateWorker(d, k, l) {
      ...
      if ('cordova' === this._exportType && this._isFileProtocol)
        return (
          (d = await this.CordovaFetchLocalFileAsArrayBuffer(l.isC3MainWorker ? d : this._scriptFolder + d)),
    ...
    async _InitWorker(d, k) {
      ...
      this._worker = await this.CreateWorker(l, this._baseUrl, { type: 'module', name: 'Runtime', isC3MainWorker: !0 })
      ...
      if ('cordova' === this._exportType)
        for (let q = 0, t = d.projectScripts.length; q < t; ++q) {
          const x = d.projectScripts[q],
            E = x[0]
          if (E === d.mainProjectScript || 'scriptsInEvents.js' === E || E.endsWith('/scriptsInEvents.js'))
        ...
        if ((t || (t = q), q === n))
          try {
            ;(t = await this._MaybeGetCordovaScriptURL(t)),
              ...
              'preview' !== this._exportType ||
                l[q] ||
                this._ReportProjectMainScriptError(q, 'main script did not run to completion')
          } catch (x) {
            this._ReportProjectMainScriptError(q, x)
          }
        else if ('scriptsInEvents.js' === q || q.endsWith('/scriptsInEvents.js'))
    ...
    PostToRuntimeComponent(d, k, l, n, r) {
      this._messageChannelPort.postMessage(
        { type: 'event', component: d, handler: k, dispatchOpts: n || null, data: l, responseId: null },
        r
      )
    }
    PostToRuntimeComponentAsync(d, k, l, n, r) {
      ...
      this._messageChannelPort.postMessage(
        { type: 'event', component: d, handler: k, dispatchOpts: n || null, data: l, responseId: q },
        r
      )
      return t
    }
    ...
    _PostResultToRuntime(d, k, l) {
      ...
      this._messageChannelPort.postMessage({ type: 'result', responseId: d, isOk: k, result: l }, n)
    }
    ...
    async _WasmDecodeWebMOpus(d) {
      d = await this.PostToRuntimeComponentAsync('runtime', 'opus-decode', { arrayBuffer: d }, null, [d])
    ...
    async _MaybeGetCordovaScriptURL(d) {
      return 'cordova' === this._exportType &&
        (d.startsWith('file:') || (this._isFileProtocol && this.IsRelativeURL(d)))
    ...
    async RequestPermissions(d) {
      ...
      for (const k of d) if (!(await this._HasPermission(k)) && !1 === (await this._RequestPermission(k))) return !1
      return !0
    }
    ...
    async _ConvertDataUriToBlobs(d, k) {
      if ('object' === typeof k)
        (this._localFileBlobs[d] = new Blob([k.str], { type: k.type })), (this._localFileStrings[d] = k.str)
      else {
    ...
    _BinaryStringToBlob(d, k) {
      ...
      let n = l >> 2,
        ...
        x
      for (x = t = 0; t < n; ++t)
        q[t] = d.charCodeAt(x++) | (d.charCodeAt(x++) << 8) | (d.charCodeAt(x++) << 16) | (d.charCodeAt(x++) << 24)
    ...
    class extends self.DOMHandler {
      constructor(f) {
        ...
        'undefined' !== typeof PointerEvent
          ? (window.addEventListener('pointerdown', y, { passive: !1 }), d.addEventListener('pointerdown', y))
        ...
        'undefined' !== typeof PointerEvent
          ? (window.addEventListener('pointerdown', g => {
            ...
            this._iRuntime.UsesWorker() && 'undefined' !== typeof window.onpointerrawupdate && self === self.top
              ...
              : window.addEventListener('pointermove', g => this._OnPointerEvent('pointermove', g)),
            window.addEventListener('pointerup', g => this._OnPointerEvent('pointerup', g)),
            window.addEventListener('pointercancel', g => this._OnPointerEvent('pointercancel', g)))
          : (window.addEventListener('mousedown', g => {
              this._HandlePointerDownFocus(g)
              this._OnMouseEventAsPointer('pointerdown', g)
            }),
            window.addEventListener('mousemove', g => this._OnMouseEventAsPointer('pointermove', g)),
      ...
      _OnWindowResize() {
        ...
        if (this._iRuntime.IsAndroidWebView()) {
          ...
          0 < this._virtualKeyboardHeight &&
            ((this._virtualKeyboardHeight = 0), this._OnAndroidVirtualKeyboardChange(g, this._virtualKeyboardHeight))
      ...
      _TrySetTargetOrientation() {
        ...
        if (screen.orientation && screen.orientation.lock)
          screen.orientation.lock(f).catch(g => console.warn('[Construct 3] Failed to lock orientation: ', g))
        else
          try {
            let g = !1
      ...
      _OnPointerEvent(f, g) {
        ...
        'mouse' === g.pointerType &&
          ((d = 'mousemove'),
          'pointerdown' === f ? (d = 'mousedown') : 'pointerup' === f && (d = 'mouseup'),
      ...
      _AttachDeviceOrientationEvent() {
        this._attachedDeviceOrientationEvent ||
          ...
          window.addEventListener('deviceorientationabsolute', f => this._OnDeviceOrientationAbsolute(f)))
      }
...
self.JobSchedulerDOM = class {
  ...
  async Init() {
    ...
    this._dispatchWorker = await this._runtimeInterface.CreateWorker(c, this._baseUrl, { name: 'DispatchWorker' })
    c = new MessageChannel()
    this._inputPort = c.port1
    this._dispatchWorker.postMessage({ type: '_init', 'in-port': c.port2 }, [c.port2])
  ...
  async _CreateJobWorker() {
    ...
    a = await this._runtimeInterface.CreateWorker(a, this._baseUrl, { name: 'JobWorker' + c })
    const b = new MessageChannel(),
      e = new MessageChannel()
    this._dispatchWorker.postMessage({ type: '_addJobWorker', port: b.port1 }, [b.port1])
    a.postMessage({ type: 'init', number: c, 'dispatch-port': b.port2, 'output-port': e.port2 }, [b.port2, e.port2])
  ...
  self.AudioDOMHandler = class extends self.DOMHandler {
    ...
    *audioInstancesByTag(a) {
    ...
    async _GetAudioInstance(a, b, e, h, m) {
      for (const p of this._audioInstances) if (p.GetUrl() === b && (p.CanBeRecycled() || m)) return p.SetTag(h), p
    ...
    async _Play(a) {
      ...
      0 < w &&
        ...
        (this._audioContext.getOutputTimestamp
          ? ((a = this._audioContext.getOutputTimestamp()), (w = w - a.performanceTime / 1e3 + a.contextTime))
        ...
        ;(this._lastAudioInstance = await this._GetAudioInstance(b, e, h, p, m)),
          v
            ? (this._lastAudioInstance.SetPannerEnabled(!0),
              this._lastAudioInstance.SetPan(v.x, v.y, v.angle, v.innerAngle, v.outerAngle, v.outerGain),
...
self.C3Html5AudioBuffer = class extends self.C3AudioBuffer {
  constructor(c, a, b, e, h) {
    ...
    this._audioElem.addEventListener('canplay', () => {
      this._loadResolve &&
        ((this._loadState = 'loaded'), this._loadResolve(), (this._loadReject = this._loadResolve = null))
  ...
  _OnError(c) {
    ...
    this._loadReject &&
      ((this._loadState = 'failed'), this._loadReject(c), (this._loadReject = this._loadResolve = null))
  }
  IsLoaded() {
...
self.C3WebAudioBuffer = class extends self.C3AudioBuffer {
  ...
  async _Fetch() {
    ...
    if ('cordova' === c.GetExportType() && c.IsRelativeURL(this._url) && c.IsFileProtocol())
  ...
  SetSuspended(c) {
    c
      ? this.IsPlaying()
        ...
        : (this._resumeMe = !1)
      : this._resumeMe && (this._audioDomHandler.TryPlayMedia(this.GetAudioElement()), (this._resumeMe = !1))
  }
}
;('use strict')
self.C3WebAudioInstance = class extends self.C3AudioInstance {
  ...
  GetPlaybackTime() {
    ...
    c = this._isPaused
      ...
      : this._playFromSeekPos + (this.GetCurrentTime() - this._playStartTime) * this._playbackRate
    this._isLooping || (c = Math.min(c, this.GetDuration()))
    return c
  }
  Play(c, a, b, e) {
  ...
  Seek(c) {
    this._isStopped ||
      this.HasEnded() ||
      (this._isPaused ? (this._resumePosition = c) : (this.Pause(), (this._resumePosition = c), this.Resume()))
  }
  GetResumePosition() {
  ...
  class extends self.DOMHandler {
    ...
    _OnReadyForSWMessages() {
      window.C3_RegisterSW &&
        ...
        window.OfflineClientInfo.SetMessageCallback(c => this.PostToRuntime('sw-message', c.data))
    }
    ...
    _OnLockOrientation(c) {
      c = c.orientation
      if (screen.orientation && screen.orientation.lock)
        screen.orientation.lock(c).catch(a => console.warn('[Construct 3] Failed to lock orientation: ', a))
      else
        try {
          let a = !1
          screen.lockOrientation
      ...
      if ('back' === a) navigator.app && navigator.app.backHistory ? navigator.app.backHistory() : window.history.back()
        ...
        self.cordova && self.cordova.InAppBrowser
          ...
          : this._isScirraArcade ||
            (2 === b ? (window.top.location = a) : 1 === b ? (window.parent.location = a) : (window.location = a))
      } else
        'new-window' === a &&
          ((a = c.url),
          (c = c.tag),
          self.cordova && self.cordova.InAppBrowser ? self.cordova.InAppBrowser.open(a, '_system') : window.open(a, c))
    }
    _OnRequestFullscreen(c) {
      if ('windows-webview2' === this._exportType || 'macos-wkwebview' === this._exportType)
        self.RuntimeInterface._SetWrapperIsFullscreenFlag(!0),
          this._iRuntime._SendWrapperMessage({ type: 'set-fullscreen', fullscreen: !0 })
      else {
    ...
    _OnExitFullscreen() {
      'windows-webview2' === this._exportType || 'macos-wkwebview' === this._exportType
        ? (self.RuntimeInterface._SetWrapperIsFullscreenFlag(!1),
          this._iRuntime._SendWrapperMessage({ type: 'set-fullscreen', fullscreen: !1 }))
  ...
  class extends self.DOMHandler {
    ...
    Init() {
      ...
      setInterval(() => {
        ...
        h !== m && ((h = m), this.PostToRuntime('clipboard-change', { 'clipboard-text': m }))
      }, 1e3)
      return Object.assign({ argv: nw.App.argv, 'clipboard-text': h }, this._GetWindowInfo())
    }
    _GetWindowInfo() {
...
;('use strict')
{
  ...
  self.RuntimeInterface.AddDOMHandlerClass(
    class extends self.DOMHandler {
      ...
      _OnReady() {
        this._isReady = !0
        for (const a of this._GetActiveGamepads()) this.PostToRuntime('gamepad-connected', { index: a.index, id: a.id })
        this._isSupported && this._StartTicking()
      }
      _OnGamepadConnected(a) {
        this._isReady && ((a = a.gamepad), this.PostToRuntime('gamepad-connected', { index: a.index, id: a.id }))
      }
      _OnGamepadDisconnected(a) {
        this._isReady && this.PostToRuntime('gamepad-disconnected', { index: a.gamepad.index })
      }
      Tick() {
        ...
        a.length &&
          ((a = a.map(b => ({
            index: b.index,
            id: b.id,
            buttons: b.buttons.map(e => ({ pressed: e.pressed, value: e.value })),
            axes: b.axes
          }))),
  ...
  self.RuntimeInterface.AddDOMHandlerClass(
    class extends self.DOMElementHandler {
      ...
      CreateElement(a, b) {
        ...
        e.addEventListener('change', () => this.PostToRuntimeElement('change', a, { value: parseFloat(e.value) }))
        e.addEventListener('input', () => this.PostToRuntimeElement('input', a, { value: parseFloat(e.value) }))
        b.id && (e.id = b.id)
